...
## 0.6 前端视觉重构（2026-09-20）

本轮只重构信息层级、视觉系统和响应式交互，不触碰数值内核、实验计算图、标准 profile、持久化 schema 与导出语义。目标是在不增加 UI 框架、图标包或字体网络依赖的前提下，把现有“功能齐但工程后台感强”的界面收敛为轻量、稳定、适合实验现场录入的工作台。

实施范围：

- 统一设计令牌：更明确的背景/表面/边框/文本层级、亮暗主题、科学蓝主色与低饱和状态色；
- 使用仓库内联 SVG 图标替换平台相关 emoji，避免不同系统显示风格不一致；
- 桌面侧栏强化分区与当前页状态，移动端保留 5 个高频入口，设置/规则来源放入移动顶部工具区；
- 首页改为任务优先：主 CTA、工作流概览、四个常用入口、当前标准摘要、最近项目；
- 实验卡强调编号、学科、报告类型与安全信息，减少 badge 堆叠；
- 工作台保持“三栏”信息架构，但提升顶部栏、步骤导航、结果检查器的视觉层级；移动端检查器使用真正的底部抽屉状态；
- 表单、按钮、Tab、表格统一交互尺寸；键盘焦点使用可见 `:focus-visible` 轮廓；
- 动效仅限短时颜色/位移过渡，并在 `prefers-reduced-motion: reduce` 下关闭；
- 不引入 Tailwind、MUI、图标 npm 包、Web Font 或运行时动画库，保持 bundle 与 CPU/内存开销基本不变。

验收标准：

1. `npm run typecheck`、`npm test`、`npm run build` 均通过，既有数值测试无回归；
2. 首页、实验、公式、数据处理、项目、设置、规则来源与实验工作台在 ≥1280 px、768–1024 px、≤600 px 三类视口均无导航遮挡和主要内容横向溢出（数据表允许自身横向滚动）；
3. 交互控件满足 WCAG 2.2 的最小目标尺寸/间距原则，所有键盘可达控件具有清晰焦点态；
4. 亮/暗主题均保持文本、边框、选中态和警告态可辨，不以颜色作为唯一语义；
5. 公式详情页不再出现空白公式块；移动端底部导航不再塞入 7 个等权入口；
6. 不增加生产依赖；设计层新增代码主要为 CSS 与小型内联 SVG；
7. 视觉改动不改变课程规则、不确定度算法、公式 provenance、项目 JSON schema 与导出结果。
...
目标：现代、克制、像“实验室工作台”，不是花哨公式站。视觉系统采用“深色导航骨架 + 中性工作区 + 科学蓝强调色”，优先保证长时间录入、公式阅读、数据表和结果检查的清晰度。

- 设计令牌只在 `src/app/styles.css` 定义，业务组件禁止各自发明新的主色、阴影和圆角体系；
- 桌面侧栏采用稳定深色底，工作区按主题切换亮/暗表面；活动项同时使用背景、文字、左侧标记/形状，不只靠颜色；
- 一级页面标题、说明、操作按钮统一为 `page-heading` 层级；首页 hero 仅使用 CSS 渐变与网格纹理，不加载装饰图片；
- 面板默认近似“纸面”而非浮动卡片：细边框 + 极轻阴影；嵌套面板取消重复阴影，避免“卡片套卡片”；
- 数学、原始数据和最终结果保持最高可读性，最终结果继续使用等宽字体但不使用夸张大字号；
- 警告/安全/错误/成功使用不同文字、图标/符号和边框语义，不以颜色作为唯一编码；
- 桌面导航与首页入口使用同一套本地 SVG 图标；不使用 emoji 作为主导航图标；
- 表格仍允许高密度，但按钮、表单和移动端操作目标不得压缩到难以点击；小按钮通过间距满足可触达性；
- 键盘导航使用 `:focus-visible` 的外轮廓，避免仅用 box-shadow；减少动画偏好的用户禁用非必要过渡与位移；
- 大量表格和结果区减少卡片嵌套，优先连续工作区；不为纯视觉效果引入 blur/backdrop-filter、Canvas 动画或额外运行时库。
