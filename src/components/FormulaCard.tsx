...
export function FormulaCard({ formula }: { formula: FormulaDefinition }) {
  ...
  return (
    <div className="panel formula-card">
      ...
      {formula.conditions && <div className="small muted"><MarkdownInline>{`**适用条件：**${formula.conditions}`}</MarkdownInline></div>}
      <div className="fc-vars">
        {formula.variables.map((v) => (
          <span key={v.name} className="var-chip" title={v.label}>
            <Tex tex={v.name} /> {v.unit ? <span className="vu"><MarkdownInline>{`[${v.unit}]`}</MarkdownInline></span> : null}
          </span>
        ))}
      </div>
      <div className="fc-actions">
        <button className="btn btn-sm btn-primary" onClick={() => navigate(`/formulas/${formula.id}`)}><MarkdownInline allowLinks={false}>计算</MarkdownInline></button>
