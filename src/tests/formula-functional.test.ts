...
import { HALL_FORMULAS } from '../formulas/hall';
...
describe('公式计算复核', () => {
  it('载流子浓度传播包含电荷常量', () => {
    const formula = HALL_FORMULAS.find(f => f.id === 'carrier-density')!;
    const result = propagateUncertainty(formula.expression, [{ symbol: 'RH', value: 2, uncertainty: 0.1 }], TSINGHUA_A1_2026.sigfig, {}, Object.fromEntries(formula.constants!.map(c => [c.name, c.value])));
    expect(result.relative).toBeCloseTo(0.05, 12);
  });
  it('霍尔灵敏度为 RH/d，且不能换算为霍尔系数', () => {
    const unit = HALL_FORMULAS.find(f => f.id === 'hall-voltage')!.variables[0].unit;
