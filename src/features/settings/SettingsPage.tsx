    ...
    <main className="page">
        ...
        <div className="card-grid">
          {profileCards.map(({ p, recommended }) => (
            ...
            >
              ...
              <div className="small muted" style={{ margin: '6px 0' }}>{p.description}</div>
              <ul className="small" style={{ margin: 0, paddingLeft: 18 }}>
                {p.rulesSummary.slice(0, 5).map((r, i) => <li key={i}>{r}</li>)}
              </ul>
          ...
          >
            <div className="fc-title"><span>自定义</span>{settings.standardProfileId === 'custom' && <Badge variant="success">当前</Badge>}</div>
            <div className="notice notice-warning" style={{ marginTop: 6 }}>
              <span className="n-icon">⚠</span><div className="n-body small">自定义规则，不代表课程或 GB/T 标准</div>
            </div>
            <div className="form-grid" style={{ marginTop: 8 }} onClick={(e) => e.stopPropagation()}>
              <label className="small">包含概率 P
              ...
              <label className="small">A 类乘 t 因子
              ...
              <label className="small">首位≥3 压缩 1 位
              ...
              <label className="small">ΔA &lt; ΔB/3 简化
            ...
            {settings.customOptions.typeAWithTFactor && settings.customOptions.bMode === 'distribution' && (
              <div className="notice notice-danger" style={{ marginTop: 8 }}>
                <span className="n-icon">✕</span><div className="n-body small">此组合把 t 因子化的 A 类与标准不确定度形式的 B 类混算，与 GB/T 相悖（AGENTS §3）。</div>
              </div>
            )}
          </div>
        </div>
        <div className="small muted" style={{ marginTop: 10 }}>
          当前生效：{profile.name} · 记号 {profile.notation.combined} · B 类 {profile.bType.mode === 'instrument-limit' ? 'ΔB=Δ仪' : '分布换算'}
          {buildCustomProfile(settings.customOptions).id === 'custom' && settings.standardProfileId === 'custom' ? '' : ''}
        </div>
      </Panel>
      ...
      <Panel title="数值与有效数字">
        <div className="form-grid">
          <div>
            <div className="field-label">最终不确定度有效数字</div>
            ...
            <div className="field-help">选择"由标准决定"时，局部组件不得覆盖（plan §3.2）</div>
          </div>
          <div>
            <div className="field-label">相对不确定度位数（标准内）</div>
            <select className="select" value={String(profile.sigfig.relativeDigits)} disabled>
              <option value="2">2 位（{profile.shortName}）</option>
            </select>
          </div>
          <div>
            <div className="field-label">角度显示</div>
        ...
        <div className="small muted" style={{ marginTop: 8 }}>修约策略集中在 core/sigfig；中间计算永远保留全精度，仅显示修约（AGENTS §4.2/§4.5）。</div>
      </Panel>

      <Panel title="单位与物理量">
        <div className="form-grid">
          <div>
            <div className="field-label">内部自动转换到 SI</div>
            <input className="input" value="始终开启" disabled />
          </div>
          <div>
            <div className="field-label">显示维度检查信息</div>
      ...
      <Panel title="图表与导出">
        <div className="form-grid">
          <div>
            <div className="field-label">默认导出格式</div>
          ...
          <div>
            <div className="field-label">Markdown 数学风格</div>
          ...
          <div>
            <div className="field-label">图表默认显示 r</div>
            <select className="select" value={settings.showFitR ? '1' : '0'} onChange={(e) => settings.set('showFitR', e.target.value === '1')}>
              <option value="1">显示</option><option value="0">隐藏</option>
            </select>
          </div>
        </div>
        <div className="small muted" style={{ marginTop: 8 }}>科学图表导出固定白底可打印样式（AGENTS §12）。</div>
      </Panel>

      <Panel title="数据与隐私">
        <div className="small" style={{ marginBottom: 8 }}>🔒 数据默认仅保存在本浏览器（IndexedDB），不上传任何服务器。</div>
        <div className="row">
          <label className="row small">
            <input type="checkbox" checked={settings.autosave} onChange={(e) => settings.set('autosave', e.target.checked)} />
            自动保存
          </label>
          <button className="btn btn-sm" onClick={() => { fileRef.current?.click(); }}>导入项目 JSON</button>
          <input
          ...
          >清空本地数据（二次确认）</ConfirmButton>
          {importCount > 0 && <span className="small muted">本次已导入 {importCount} 个项目</span>}
        </div>
      </Panel>
      ...
      <Modal open={rulesOpen} onClose={() => setRulesOpen(false)} title={`${profile.name} 规则详情`} wide>
        ...
        <ul style={{ paddingLeft: 18 }}>
          {profile.rulesSummary.map((r, i) => <li key={i} style={{ margin: '4px 0' }}>{r}</li>)}
        </ul>
        <div className="small muted">来源：{profile.source}</div>
      </Modal>
    </main>
  );
}
