...
export function ForcedPlotsBlock({ series }: { series?: { name: string; points: { x: number; y: number }[]; phiPoints: { x: number; y: number }[] }[] }) {
  const data = series ?? [];
  if (data.length === 0) {
    return <Panel title="受迫振动特性图"><div className="empty-state"><div><MarkdownInline>填写受迫数据并完成自由衰减部分后显示</MarkdownInline></div></div></Panel>;
  }
...
export function QuasiDiagnosticsBlock({ data }: { data?: {
  ...
  if (!data) return <Panel title="准稳态诊断"><div className="field-help"><MarkdownInline>填写时序数据后显示</MarkdownInline></div></Panel>;
  ...
  return (
    <Panel title="准稳态区间诊断（客观提示，区间由你确认）">
      <table className="contrib-table">
        <tbody>
          <tr><td><MarkdownInline>区间内行数</MarkdownInline></td><td className="num">{data.rowsIn} / {data.rowsAll}</td></tr>
          <tr><td><MarkdownInline>U1 均值（修正后）</MarkdownInline></td><td className="num">{data.u1Mean.toPrecision(8)} μV</td></tr>
          <tr><td><MarkdownInline>U1 标准差</MarkdownInline></td><td className="num">{Number.isFinite(data.u1Std) ? `${data.u1Std.toPrecision(6)} μV` : '—'}</td></tr>
          <tr><td><MarkdownInline>U1 时间斜率 / U2 斜率</MarkdownInline></td><td className="num">{data.u1SlopeRel.toPrecision(4)}（应 ≈0）</td></tr>
          <tr><td><MarkdownInline>U2–τ 线性拟合 r</MarkdownInline></td><td className="num">{data.u2Fit.r.toPrecision(6)}（应 ≈1）</td></tr>
        </tbody>
      </table>
      <div className={`notice ${u1Flat && u2Linear ? 'notice-success' : 'notice-warning'}`} style={{ marginTop: 8 }}>
        ...
        <div className="n-body">
          <MarkdownInline>{u1Flat && u2Linear
            ? 'U1 近似恒定且 U2 对时间近似线性：当前区间满足准稳态判据。'
            : '判据未全部满足：U1 应近似恒定、U2 应近似线性。请调整 `τstart/τend`（候选区间仅供参考，由你确认）。'}</MarkdownInline>
        </div>
      </div>
    </Panel>
  );
      ...
      <div className="stack">
        {MICHELSON_CHECKLIST.map((item) => (
          <label key={item.id} className="row" style={{ cursor: 'pointer', gap: 8 }}>
            ...
            <span className="small muted"><MarkdownInline>{item.tip}</MarkdownInline></span>
          </label>
        ))}
      </div>
