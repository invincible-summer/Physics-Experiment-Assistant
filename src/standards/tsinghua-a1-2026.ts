...
export const TSINGHUA_A1_2026: StandardProfile = {
  ...
  rulesSummary: [
    ...
    '$\\Delta_A<\\Delta_{\\text{仪}}/3$ 时允许简化为 $\\Delta=\\Delta_{\\text{仪}}$（界面会提示"采用课程简化规则"）',
    '不确定度一般取 2 位有效数字；首位 $\\ge 3$ 时可取 1 位',
    '相对不确定度取 2 位有效数字',
    '最终测量值末位与不确定度末位对齐',
    '间接量按独立输入的偏导方和根传播：$\\Delta_Y=\\sqrt{\\sum_i \\left(\\dfrac{\\partial f}{\\partial x_i}\\,\\Delta_{x_i}\\right)^2}$',
    '已定系统误差先行修正，再报告估计值',
  ],
